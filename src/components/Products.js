    ...
    return (
        <Box sx={{ width: "calc(100% - 1rem)", margin: "1rem auto 4rem auto" }} className="products">
            <Box sx={{ width: "100%" }}>
                ...
                <Grid container>
                    {/* -item-- */}
                    {products.loading == true ? <Box sx={{ display: "flex", justifyContent: "center", padding: "2rem 0", width: "100%" }}> <Spinner /> </Box> : products.servicesData.map((e) => {
                        return (
                            <Grid item xs={6}>
                                <Box sx={{ width: "calc(100% - 10px)", boxShadow: "0px 0px 10px lightgray", margin: "auto auto 10px auto", borderRadius: "10px", overflow: "hidden" }}>
                                    <img src={e.image} alt="#" style={{ width: "100%", aspectRatio: "16 / 14", objectFit: "contain" }} className="img-fluid" onClick={() => { navigate(`/productDetails/${e.id}`) }} />
                                    <Box sx={{ padding: "10px" }}>
